return {
  ...
  config = function()
    ...
    print("✅ Codeium Tab mapping should now work!")
    ...
    vim.defer_fn(function()
      ...
      for _, map in ipairs(maps) do
        if map.lhs == '<Tab>' and map.desc and map.desc:match('[Cc]odeium') then
          ...
          print("✅ Codeium Tab mapping confirmed:", map.desc)
          break
        end
      end
      
      if not found_codeium_tab then
        print("⚠️  Codeium Tab mapping not found - manual setup needed")
        -- Manually set up Codeium Tab if it didn't work
      ...
      print("ℹ️  Codeium status:", status)
    end, 1000)
