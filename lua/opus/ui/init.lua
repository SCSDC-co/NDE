...
local function refresh_tasks()
    if not tasks_popup then return end
...
local function show_input(title, default_text, callback)
    local input = Input({
        ...
        on_close = function()
            if tasks_popup then vim.api.nvim_set_current_win(tasks_popup.winid) end
        end,
        on_submit = function(value)
            if value and #value > 0 then callback(value) end
            if tasks_popup then
...
function ui.show_tasks()
    ...
    local layout = Layout({
        ...
        Layout.Box(tasks_popup, { size = "100%" }),
        Layout.Box(help_popup, { size = help_popup.height }),
    }, { dir = "col" }))
    ...
    local function update_scroll()
        ...
        if not tasks_popup or not tasks_popup.winid then return end
        local win_height = vim.api.nvim_win_get_height(tasks_popup.winid)
        if win_height == 0 then return end
    ...
    tasks_popup:map("n", "a", function()
        show_input("Add New Task", "", function(name) core.add_task({ name = name, completed = false, tags = {} }) end)
    end, keymap_opts)

    tasks_popup:map("n", "r", function()
        if #core.tasks > 0 then show_input("Rename Task", core.tasks[selected_task].name, function(name) core.rename_task(selected_task, name) end) end
    end, keymap_opts)
    ...
    tasks_popup:map("n", "t", function()
        if #core.tasks > 0 then show_input("Add Tag", "", function(tag) core.add_tag(selected_task, tag) end) end
    end, keymap_opts)
    ...
    tasks_popup:map("n", {"q", "<Esc>"}, function() layout:unmount() active_layout = nil end, keymap_opts)
